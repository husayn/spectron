...
function Application (options) {
  ...
  this.launchWithChromeOpts = options.launchWithChromeOpts === false ? false : true;
  this.onBeforeLaunchScript = options.onBeforeLaunchScript || '';
  ...
  this.ignoreBinaryPath = options.ignoreBinaryPath
...
Application.prototype.exists = function () {
  var self = this
  return new Promise(function (resolve, reject) {
    // Binary path is ignored by ChromeDriver if debuggerAddress is set. Adding a flag to ignore binary path for things like protocol launch.
    if (self.debuggerAddress || self.ignoreBinaryPath) return resolve()
...
Application.prototype.createClient = function () {
  ...
  return new Promise(function (resolve, reject) {
    var args = []
    if (self.onBeforeLaunchScript) {
      args.push('on-before-launch-script=' + self.onBeforeLaunchScript)
    }
    args.push('launch-with-chrome-opts=' + self.launchWithChromeOpts);
